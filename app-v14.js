...
(function(){
  ...
  let paused=false, pauseStarted=0, pausedMs=0;
  ...
  function dashboard(){
    if(!$('homeMetrics')||!$('lastRide'))return;
  ...
  function togglePause(){
    if(!paused){paused=true;pauseStarted=Date.now();if(watchId!==null)navigator.geolocation.clearWatch(watchId);watchId=null;$('pauseRide').textContent='▶ Weiter';$('gpsStatus').textContent='PAUSIERT'}
    else{paused=false;pausedMs+=Date.now()-pauseStarted;$('pauseRide').textContent='Ⅱ Pause';$('gpsStatus').textContent='GPS WIRD GESUCHT…';if(navigator.geolocation)watch(update)}
  }
  function showSummary(){
    const r=routes()[0];if(!r)return;
    const modal=document.createElement('div');modal.className='summary-modal';modal.innerHTML=`<div class="summary-card"><div class="summary-kicker">FAHRT BEENDET</div><h2>Sauber gefahren. 🛴</h2><div class="summary-grid"><div><span>Distanz</span><b>${(r.distance/1000).toFixed(2)} km</b></div><div><span>Zeit</span><b>${fmtMs(Math.max(0,r.duration-pausedMs))}</b></div><div><span>Max.</span><b>${Number(r.maxSpeed||0).toFixed(1)} km/h</b></div><div><span>Punkte</span><b>${r.points?.length||0}</b></div></div><input id="summaryName" value="${esc(r.name)}" maxlength="40"><button id="summarySave" class="primary">✓ Route speichern</button><button id="summaryClose" class="secondary">Zur Routenübersicht</button></div>`;
  ...
  function bind(){
    injectHome();
    if($('startRide'))$('startRide').onclick=window.startTracking;
    if($('stopRide'))$('stopRide').onclick=window.stop;
    setInterval(()=>{if($('homeScreen')&&!$('homeScreen').classList.contains('hidden'))dashboard()},5000);
  }
